...
public class NpcCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command cmd,
        ...
        switch (args[0].toLowerCase()) {
            case "create" -> {
                ...
                String name = args.length > 1 ? String.join(" ", Arrays.copyOfRange(args, 1, args.length)) : "NPC";
                plugin.getNpcManager().createNpc(p, name);
            }
            case "delete" -> {
    ...
    private void sendHelp(Player p) {
        ...
        if (p.hasPermission("auto.npc.create"))  p.sendMessage(ColorUtils.color("&e/anpc create <назва>"));
